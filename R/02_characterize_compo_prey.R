...
boxplot_compo_prey_sp <- function(res_prey_tib) {
  ...
  res_prey_tib |>
    ...
    tidyr::pivot_longer(cols = c(Ca, Co, Cu, Fe, K, Mg, Mn, Na, Ni, P, Se, Sr, Zn), 
                        names_to = "Nutrient", 
                        values_to = "concentration_mg_g_dw") |>
    dplyr::filter(!(Nutrient %in% c("As", "Cd", "Pb", "V", "Ag"))) |>
    # remove NAs if there is still some
    dplyr::filter(!(is.na(concentration_mg_g_dw))) |>
    ggplot2::ggplot(ggplot2::aes(x = Species, y = concentration_mg_g_dw, fill = Species)) +
    ggplot2::geom_boxplot() +
    ggplot2::facet_wrap(~ Nutrient, scale = "free") +
...
boxplot_compo_prey_genus <- function(res_prey_tib) {
  ...
  res_prey_tib |>
    ...
    # remove NAs if there is still some
    dplyr::filter(!(is.na(concentration_mg_g_dw))) |>
    ggplot2::ggplot(ggplot2::aes(x = Genus, y = concentration_mg_g_dw, fill = Genus)) +
    ggplot2::geom_boxplot() +
    ggplot2::facet_wrap(~ Nutrient, scale = "free") +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 20, hjust = 1))
...
boxplot_compo_prey_fam <- function(res_prey_tib) {
  ...
  res_prey_tib |>
    ...
    # remove NAs if there is still some
    dplyr::filter(!(is.na(concentration_mg_g_dw))) |>
    ggplot2::ggplot(ggplot2::aes(x = Family, y = concentration_mg_g_dw, fill = Family)) +
    ggplot2::geom_boxplot() +
    ggplot2::facet_wrap(~ Nutrient, scale = "free") +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 20, hjust = 1))
...
boxplot_compo_prey_camp <- function(res_prey_tib) {
  ...
  res_prey_tib |>
    ...
    # remove NAs if there is still some
    dplyr::filter(!(is.na(concentration_mg_g_dw))) |>
    ggplot2::ggplot(ggplot2::aes(x = campaign, y = concentration_mg_g_dw, fill = campaign)) +
    ggplot2::geom_boxplot() +
    ggplot2::facet_wrap(~ Nutrient, scale = "free") +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 20, hjust = 1))
...
PCA_prey<- function(res_prey_tib) {
  ...
  data.act <- res_prey_tib |>
    ...
    # tidyr::pivot_longer(cols = c(As, Ca, Cd, Co, Cu, Fe, K, Mg, Mn, Na, Ni, P, Pb, Se, Sr, Zn), 
    #                     names_to = "Element", 
